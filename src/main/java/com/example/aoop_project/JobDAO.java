...
public class JobDAO {
    ...
    public void create(Job job) throws SQLException {
        String sql = "INSERT INTO job (poster_id, title, description, location, salary_range, tech_stack, job_type, requirements, posted_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public List<Job> findByPosterId(int posterId) throws SQLException {
        String sql = "SELECT * FROM job WHERE poster_id = ? ORDER BY posted_at DESC";
        List<Job> jobs = new ArrayList<>();
    ...
    public List<Job> searchJobs(String keyword, String jobType, String location, String techStack) throws SQLException {
        StringBuilder sql = new StringBuilder("SELECT * FROM job WHERE 1=1");
