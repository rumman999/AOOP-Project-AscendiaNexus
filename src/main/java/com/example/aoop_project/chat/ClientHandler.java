...
public class ClientHandler implements Runnable {
    ...
    private void sendPrivateMessage(int receiverId, String msg) {
        ...
        if (target != null) {
            target.send(username + " (private): " + msg);
            this.send("You to " + target.username + ": " + msg);
    ...
    private void sendGroupMessage(int groupId, String msg) {
        // Simple: broadcast to all for now (later filter by group membership in DB)
        for (ClientHandler client : ChatServer.clients.values()) {
            client.send("[Group " + groupId + "] " + username + ": " + msg);
        }
